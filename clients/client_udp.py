...
class EchoClientDatagramProtocol(DatagramProtocol):
    ...
    #f = open('data/client_' + str(id) + '.dat', "a") 
    ...
    def datagramReceived(self, datagram, host):
        ...
        type_pck, window, seq_number = (struct.unpack('!bbl',datagram[0:6]))
        ...
        if (seq_number != self.expected_seq and type_pck == 1):
            ...
            self.recv_bytes += len(datagram)    
            #self.f.write(datagram[6:].decode('utf-8'))
...
def main(server_,port_,file_, window_, id_):
	...
	# Creating a new file to store received data
    #f = open('data/client_' + str(id_) + '.dat', "w")
    #f.close() 
