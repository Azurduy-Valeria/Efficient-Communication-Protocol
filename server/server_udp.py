...
parser.add_argument("-b", "--probability", type=float, help="Probability", default='0.01')
parser.add_argument("-t", "--timeout", type=float, help="Timeout", default='0.1')
parser.add_argument("-p", "--port", type=int, help="Server port", default='9999')



def create_packet(packet_type, window_size,seq_number):
	packet = struct.pack('!bbL', packet_type,window_size, seq_number)
	return packet

def recv_and_unpack(data):
	# Now lets unpack the data. Two first bytes are headers (type and window size)
	type_pck, window, seq_number = (struct.unpack('!bbL',data[0:6]))
	payload = data[6:]
...
def main ():
	print ("Simulating this parameters", PROBABILITY,TIMEOUT_SEC)
	...
	portSocket.bind(('127.0.0.1', PORT_SERVER))
...
if __name__ == "__main__":
	...
	PROBABILITY = 1- args.probability
